...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	if (fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		{
			MessageBox(NULL, "Failed to setup proper Windows path!", "GWLoader", MB_ICONERROR | MB_OK);
			return FALSE;
		}
		...
		if (!winmmDll) {
			MessageBox(NULL, "Failed to load winmm.dll!", "GWLoader", MB_ICONERROR | MB_OK);
			return FALSE;
		}
		...
		if (strstr(GetCommandLine(), "--no-mods") != NULL)
			return TRUE;
		if (strstr(GetCommandLine(), "--loader.debug") != NULL)
			ConsoleUtils::CreateConsole();
		GWLoader::Init_Loader();
	}
	else if (fdwReason == DLL_PROCESS_DETACH)
	{
		GWLoader::Destroy_Loader();
		FreeLibrary(selfDll);
	}
	return TRUE;
}
